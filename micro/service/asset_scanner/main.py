...
def main():
    ...
    example = "\n\nexample:\n\t[-] 向注册中心(127.0.0.1:8500)注册主机(192.168.1.11:50051)服务:\n\t" \
        "python3 agent.py agent --registry_host 172.31.50.249 --registry_port 8500 --server_name asset_scanner --server_addr 172.31.50.249\n\t" \
        "\n\nexample:\n\t[-] 离线扫描脚本:\n\t" \
        "python3 agent.py  --registry_host 172.31.50.249 --registry_port 8500 --server_name asset_scanner --server_addr 172.31.50.249\n\t"
    description += example
    parser = argparse.ArgumentParser(description=description, prog='python3 main.py', formatter_class=RawTextHelpFormatter)
    # parser = argparse.ArgumentParser(prog='python3 main.py')
    subparsers = parser.add_subparsers(help='sub-command help')
    # 添加子命令 agent
    parser_a = subparsers.add_parser('agent', help='agent help')
    parser_a.add_argument('--registry_host', required=True, help='\t输入注册中心IP(必填)')               # 添加--verbose标签，标签别名可以为-v，这里action的意思是当读取的参数中出现--verbose/-v的时候
    ...
    parser_s.add_argument('--domain', required=False, help='\t目标域名: example.com (选填), domain与ip必选其一')               # 添加--verbose标签，标签别名可以为-v，这里action的意思是当读取的参数中出现--verbose/-v的时候
