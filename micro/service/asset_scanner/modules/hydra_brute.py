...
class HydraScanner:
    def __init__(self, service: str, args: str, target_list: list, username_list: list, password_list: list):
        self.target_list = target_list
        self.service = service  # 单个服务
        self.username_list = list(set(username_list))
        self.password_list = list(set(password_list))
        self.args = args
    ...
    def _format_resp(self):
        result_list = []
        pattern_res = re.compile(r'(\[\d+\]\[%s\]\shost:\s.*?)\n' % self.service)
        pattern_ip = re.compile(r'host:\s(\d+\.\d+\.\d+\.\d+:?\d*)\s')
        pattern_domain = re.compile(r'host:\s([a-z.]+)\s')
        pattern_username = re.compile(r'login:\s(.+?)\s+password:')
        pattern_password = re.compile(r'password:\s(.+?)$')
        re_result = re.findall(pattern_res, str(self.stdout, encoding='utf-8'))
        print(self.stdout)
        for res in re_result:
            try:
                ...
                result = {
                    ...
                    'command': 'hydra -w 30 -t 4 %s -l %s -p %s %s' % (self.args, username, password, self.service),
                }
...
if __name__ == "__main__":
    target_list = ['172.31.50.249:3306']
    service = 'mysql'
    username_list = ['root']
    password_list = ['123456']
    args = ''
    for _ in range(10):
        start = HydraScanner(service, args, target_list, username_list, password_list)
        result = start.run()
        # start._format_args()
